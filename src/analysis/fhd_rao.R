...
tisza_2m_r_sub=subset(tisza_2m_r,select=c(48,8,44,13,14,47,
                                          31,20,24,18,28,29))
names(tisza_2m_r_sub) <- c("lake","class","FHD_rao","x","y","type",
                           "V_fhd","C_er","C_negopp","V_sigmaz","C_ppr","V_fhdrao")

tisza_2m_r_sub=tisza_2m_r_sub[is.na(tisza_2m_r_sub$class)==FALSE,]

ferto_2m_r_sub=subset(ferto_2m_r,select=c(47,8,43,13,14,46,
                                          31,20,24,18,28,29))
names(ferto_2m_r_sub) <- c("lake","class","FHD_rao","x","y","type",
                           "V_fhd","C_er","C_negopp","V_sigmaz","C_ppr","V_fhdrao")

balaton_2m_r_sub=subset(balaton_2m_r,select=c(47,8,43,13,14,46,
                                              31,20,23,18,27,28))
names(balaton_2m_r_sub) <- c("lake","class","FHD_rao","x","y","type",
                             "V_fhd","C_er","C_negopp","V_sigmaz","C_ppr","V_fhdrao")
...
lm_fhd<-lm(FHD_rao~V_fhdrao,data=merged)
summary(lm_fhd)

lm_fhd<-lm(FHD_rao~V_fhdrao+C_negopp+V_sigmaz+C_ppr,data=merged)
summary(lm_fhd)
...
lm_fhd_fwf<-lm(FHD_rao~V_fhdrao,data=merged[merged$lake!="Lake Balaton",])
summary(lm_fhd_fwf)

lm_fhd_fwf<-lm(FHD_rao~V_fhdrao+C_negopp+V_sigmaz+C_ppr,data=merged[merged$lake!="Lake Balaton",])
summary(lm_fhd_fwf)
...
lm_fhd_nfwf<-lm(FHD_rao~V_fhdrao,data=merged[merged$lake=="Lake Balaton",])
summary(lm_fhd_nfwf)

lm_fhd_nfwf<-lm(FHD_rao~V_fhdrao+C_negopp+V_sigmaz+C_ppr,data=merged[merged$lake=="Lake Balaton",])
summary(lm_fhd_nfwf)
