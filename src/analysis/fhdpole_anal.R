...
tisza_2m_r_sub=subset(tisza_2m_r,select=c(34,7,11,12,13,33,
                                          17,23,19,27,20,30))
names(tisza_2m_r_sub) <- c("lake","class","FHD_p","x","y","type",
                           "V_sigmaz","C_negop","C_er","C_ppr","V_ewidth","V_fhd")

tisza_2m_r_sub=tisza_2m_r_sub[is.na(tisza_2m_r_sub$class)==FALSE,]

ferto_2m_r_sub=subset(ferto_2m_r,select=c(34,7,11,12,13,33,
                                          17,23,19,27,20,30))
names(ferto_2m_r_sub) <- c("lake","class","FHD_p","x","y","type",
                           "V_sigmaz","C_negop","C_er","C_ppr","V_ewidth","V_fhd")

balaton_2m_r_sub=subset(balaton_2m_r,select=c(34,7,11,12,13,33,
                                              17,22,19,26,26,30))
names(balaton_2m_r_sub) <- c("lake","class","FHD_p","x","y","type",
                             "V_sigmaz","C_negop","C_er","C_ppr","V_ewidth","V_fhd")

balaton_2m_r_sub$V_ewidth<-NA

tisza_2m_r_sub_leafon=subset(tisza_2m_r_leafon,select=c(34,7,11,12,13,33,
                                                        17,23,19,27,20,30))
names(tisza_2m_r_sub_leafon) <- c("lake","class","FHD_p","x","y","type",
                                  "V_sigmaz","C_negop","C_er","C_ppr","V_ewidth","V_fhd")
...
merged=merged[complete.cases(merged[,8]), ]
summary(merged)

round(cor(merged[,c(7,8,9,10,12)], method="spearman"),2) #without echowidth
round(cor(merged[merged$lake!="Lake Balaton",c(7:12)], method="spearman"),2) #with ecowidth -> drop ppr

# linear regression across all lakes

lm_lai<-lm(FHD_p~V_fhd,data=merged)
summary(lm_lai)
...
lm_LAI_fwf<-lm(FHD_p~V_fhd,data=merged[merged$lake!="Lake Tisza",])
summary(lm_LAI_fwf)
...
merged$id=seq(1,156,1)

ggplot(data=merged, aes(x=V_fhd, y=FHD_p),show.legend = TRUE) +  
