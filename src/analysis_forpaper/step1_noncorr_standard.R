...
for(i in rad) {
  ...
  vifcorr_pole=vifcor(pole_data_filt[,c(4:6,9:17,24)], th=0.6, method='spearman')
  print(vifcorr_pole)
  ...
  plot_data=read.csv(paste("Plot_db_",i,".csv",sep=""))
  plot_data_filt=plot_data[plot_data$nofveg>2,]
  
  vifcorr_plot=vifcor(plot_data_filt[,c(3:5,8:16,24)], th=0.6, method='spearman')
  print(vifcorr_plot)
